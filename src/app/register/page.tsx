...
export default function RegisterPage() {
  ...
  return (
    <div className="h-screen w-full flex items-center justify-center p-6 relative overflow-hidden bg-background">
      <div className="absolute top-[-5%] right-[-5%] w-[50%] h-[50%] bg-primary/20 blur-[100px] rounded-full" />
      <div className="absolute bottom-[-5%] left-[-5%] w-[50%] h-[50%] bg-accent/20 blur-[100px] rounded-full" />
      ...
      <div className="w-full max-w-[400px] relative z-10 space-y-6 flex flex-col items-center">
        <div className="text-center space-y-2">
          ...
          <p className="text-muted-foreground font-medium text-[9px] uppercase tracking-widest opacity-60">Tu nutrición inteligente</p>
        </div>
        ...
        <Card className="glass border-none shadow-2xl rounded-[3rem] overflow-hidden w-full">
          <CardHeader className="space-y-1 pb-0 pt-8 px-8">
            <CardTitle className="text-2xl font-headline font-bold text-center">Registro</CardTitle>
          </CardHeader>
          <form onSubmit={handleRegister}>
            <CardContent className="space-y-4 px-8 pt-6 pb-6">
              <div className="space-y-1.5">
                <Label className="text-[10px] font-bold uppercase tracking-[0.2em] text-muted-foreground px-1">Tu Nombre</Label>
              ...
              <Button 
                ...
                disabled={loading}
              >
                {loading ? <Loader2 className="animate-spin mr-2 h-5 w-5" /> : "Crear Cuenta"}
              </Button>
            </CardContent>
          </form>
          <CardFooter className="flex flex-col gap-2 border-t border-white/5 pt-6 pb-8">
            <p className="text-xs text-muted-foreground font-medium">
              ¿Ya eres usuario? <Link href="/login" className="text-primary font-bold hover:underline">Acceder</Link>
            </p>
          </CardFooter>
        </Card>
      </div>
